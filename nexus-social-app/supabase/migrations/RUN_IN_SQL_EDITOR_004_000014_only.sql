-- Run manually in Supabase SQL Editor — Migration 000014 (Agency hierarchy)
-- Same content as supabase/migrations/20260624_000014_agencies_hierarchy.sql
...
RETURNS boolean LANGUAGE sql STABLE SECURITY DEFINER SET search_path = public AS $$
  SELECT EXISTS (
    ...
    WHERE am.user_id = user_uuid AND am.role = 'tenant_admin' AND a.tenant_id = tenant_uuid
  );
$$;
...
RETURNS boolean LANGUAGE sql STABLE SECURITY DEFINER SET search_path = public AS $$
  SELECT EXISTS (
    SELECT 1 FROM agency_members am
    WHERE am.user_id = user_uuid AND am.agency_id = agency_uuid
...
RETURNS boolean LANGUAGE sql STABLE SECURITY DEFINER SET search_path = public AS $$
  SELECT EXISTS (
    SELECT 1 FROM brands b WHERE b.id = brand_uuid AND (
      ...
      OR EXISTS (
        SELECT 1 FROM workspaces w WHERE w.id = b.workspace_id
...
DO $$ BEGIN
  CREATE POLICY "agencies_tenant_isolation" ON agencies FOR SELECT USING (
    ...
    OR EXISTS (SELECT 1 FROM agency_members am WHERE am.agency_id = agencies.id AND am.user_id = auth.uid())
    OR EXISTS (
...
DO $$ BEGIN
  CREATE POLICY "brands_agency_isolation" ON brands FOR SELECT USING (
    ...
    OR EXISTS (
      SELECT 1 FROM workspaces w WHERE w.id = brands.workspace_id
...
DO $$ BEGIN
  CREATE POLICY "ai_cmo_campaigns_agency_isolation" ON ai_cmo_campaigns FOR SELECT USING (
    public.is_workspace_member(auth.uid(), workspace_id)
    OR (brand_id IS NOT NULL AND public.can_access_brand(auth.uid(), brand_id))
    OR EXISTS (
      SELECT 1 FROM workspaces w WHERE w.id = ai_cmo_campaigns.workspace_id
        AND public.is_tenant_admin(auth.uid(), w.tenant_id)
    )
  );
EXCEPTION WHEN duplicate_object THEN NULL; END $$;
...
DO $$ BEGIN
  CREATE POLICY "agency_members_self" ON agency_members FOR SELECT USING (
    ...
    OR EXISTS (
      SELECT 1 FROM agencies a WHERE a.id = agency_members.agency_id
